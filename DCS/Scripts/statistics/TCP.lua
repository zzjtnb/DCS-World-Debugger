...
function send_data(payload)
  ...
  payload = ZZJT.toJson(payload)
  if not payload then
    return false
  end
  -- payload = payload .. 'qiut\r\n'
  -- client:send(payload)
  payload = ZZJT.stringSlice2Table(payload, BLOCKSIZE)
...
function create_tcp_client()
  local status, error = pcall(function()
    ...
    client = socket.try(socket.connect(lhost, lport))
  end)
  if status then
...
function server_accept()
  ...
  if sock ~= nil then
    ...
    if not err then
      local status, decodedRequest = ZZJT.json2lua(line)
      if status then
...
function handleDebug(request)
  ...
  if request.payload.type == 'loadstring' then
    result, status = ZZJT.safe_loadstring(request.payload.content)
  end
  ...
  local response = {
    ...
    payload = {
      ...
      status = status,
      date = ZZJT.getTimeStamp(),
    },
  }
  if not status then
    if ZZJT.isempty(result) then
      result = "执行失败"
    end
